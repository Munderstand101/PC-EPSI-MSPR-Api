...
class ChatController extends AbstractController
{
    ...
    public function listChats(SerializerInterface $serializer,$id,UserRepository $userRepository,ConversationRepository $conversationRepository)
    {
        ...
        return $this->json($conversationRepository->findBy(["startedBy"=>$user]), 200, [], ['groups' => 'read']);
    }
    ...
    {
        $user = $conversationRepository->findOneBy(['id' => $id]);
        return $this->json($messageRepository->findBy(["conversation" => $user]), 200, [], ['groups' => 'read']);
    }


    #[Route('/start-conv/{id}', name: 'start_conv', methods: ["POST"])]
    ...
    public function sendMessage(Request $request, UserRepository $userRepository, $id)
    {
